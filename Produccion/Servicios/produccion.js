...
import { Console, info, log } from "node:console";
import { OrdenesModel } from "../../Ordenes/Servicios/ordenes.js";
import { of } from "puppeteer-core/lib/esm/third_party/rxjs/rxjs.js";
...
export class ProduccionModel {
  ...
  static async getOrdenesById(info) {
    ...
    try {
      ...
      let [modelos] = await conn.execute(`
        ...
        WHERE t1.id_orden_CAB = ?
        GROUP BY t1.idx,t1.id_orden_CAB,t1.id_receta_CAB,t1.idx_color,t1.color_modelo,t1.cantidad_modelo,t1.pricemodel
      `,[info.id])
    ...
    //     t1.cantidad_modelo,
    //     t1.pricemodel,
    //     CASE WHEN COUNT(t2.talla) > 0
    //       THEN JSON_OBJECTAGG(t2.talla, t2.cantidad)
    //       ELSE JSON_OBJECT()
    //     END as fracciones,
    //     CASE WHEN COUNT(t3.sku) > 0
    //       THEN JSON_OBJECTAGG(t2.talla, t3.sku)
    //       ELSE JSON_OBJECT()
    //     END as sku
    //   FROM tbl2_fases_prod_modelos t1
    //   LEFT JOIN tbl2_fases_prod_modelos_fracciones t2 ON t1.idx = t2.id_modelo_CAB 
    //   LEFT JOIN tbl2_subproductos t3 ON t3.idx_CAB_PROD = t1.id_receta_CAB
    //     AND t3.idx_CAB_COLOR = t1.idx_color
    //     AND t3.idx_talla = t2.id_talla_CAB 
    //   WHERE t1.id_orden_CAB = ?
    //   GROUP BY t1.idx,t1.id_orden_CAB,t1.id_receta_CAB,t1.idx_color,
    //           t1.color_modelo,t1.cantidad_modelo,t1.pricemodel
    // `,[info.id])
    ...
    try {
      ...
      const [results2] = await conn.query(`
        SELECT 
          ...
          concat('({"',
            REPLACE(
              GROUP_CONCAT(concat(talla,'":',CAST(cantidad as unsigned))),
              ',',
              ',"'
            ),
          '})') as fracciones
        FROM tbl2_guias_traslado_det_fracciones 
        WHERE id_guia_DET IN (?) 
        GROUP BY id_guia_DET
      `, [ids])
      ...
      let new_articulos = pp.map(row => {
        const match = results2.find(row2 => row2.id_guia_DET === row.idx);
        const add = match ? eval(match.fracciones) : {};
        return { ...row, ...add }
      })
  ...
  static async saveInfoGuias(data) {
    ...
    try {
      ...
      /////////////////////////////////////
      /////////////////////////////////////
      const [columns] = await conn.query("SHOW COLUMNS FROM tbl2_guias_traslado_cab");
      let validFields = columns.map(c => c.Field);

      if (data.id) {
        let cleanData = Object.keys(cabecera).reduce((c,v)=>{
          if(validFields.includes(v) && v !== 'idx') c[v] = cabecera[v]
          return c
        },{})

        await conn.query(`UPDATE tbl2_guias_traslado_cab SET ${Object.keys(cleanData).map(r=> r + '=NULLIF(?, "")').join(',')} WHERE idx = ?`,[...Object.values(cleanData),parseInt(data.id)])
        ...
        let cleanData = Object.keys(cabecera).reduce((c,v)=>{
          if(validFields.includes(v)) c[v] = cabecera[v]
          return c
        },{})

        const [res] = await conn.query(`INSERT INTO tbl2_guias_traslado_cab(${Object.keys(cleanData).join(',')}) VALUES(${Object.keys(cleanData).map(r=>'NULLIF(?,"")').join(',')})`,Object.values(cleanData))

        let [rss] = await conn.execute(`select *from tbl2_guias_traslado_cab where idx = ?`,[res.insertId])
        //console.log("La info insertada fue :",rss)
  ...
  static async saveInfoMuestras(data) {
    ...
    const penalidades = data.penalidades ? JSON.parse(data.penalidades) : []
    const reprogramacion = data.reprogramacion ? JSON.parse(data.reprogramacion) : []
    let tallasbase = ['st','xs','s','m','l','xl','xxl']
    ...
    try {
      ...
      const [infotallas] = await conn.execute("select *from tbl2_fases_prod_ordenes t1 join tbl2_tallas_template t2 on t1.tallasbase = t2.idx where t1.idx = ?",[cabecera.id_orden_CAB ?? 0])
      if(infotallas.length) tallasbase = infotallas[0].tallas.map(row=>row.desc)

      if (data.id) {
        await conn.query('UPDATE tbl2_guias_traslado_cab SET orden_ref=NULLIF(?, ""),tipo=NULLIF(?, ""),id_proveedor_CAB=NULLIF(?, ""),proveedor=NULLIF(?, ""),servicio=NULLIF(?, ""),fec_emision=NULLIF(?, ""),fec_retorno=NULLIF(?, ""),fec_recepcion=NULLIF(?, ""),costo=NULLIF(?, ""),observaciones=NULLIF(?, ""),estado=NULLIF(?, ""),motivo_traslado=NULLIF(?, ""),responsable=NULLIF(?, ""),modelo=NULLIF(?, ""),marca=NULLIF(?, ""),producto=NULLIF(?, ""),destino=NULLIF(?, ""),id_orden_CAB=? WHERE idx = ?', [cabecera.orden_ref, cabecera.tipo, cabecera.id_proveedor_CAB, cabecera.proveedor, cabecera.servicio, cabecera.fec_emision, cabecera.fec_retorno, cabecera.fec_recepcion, cabecera.costo, cabecera.observaciones, cabecera.estado, cabecera.motivo_traslado, cabecera.responsable, cabecera.modelo, cabecera.marca, cabecera.producto, cabecera.destino, cabecera.id_orden_CAB || 0 , parseInt(data.id)])
        const [res, fld] = await conn.query("SELECT *FROM tbl2_guias_traslado_det WHERE id_guia_CAB = " + parseInt(data.id))
        const ids_delete = res.filter(row => row.idx !== '' && !articulos.map(fila => parseInt(fila.idx)).includes(parseInt(row.idx)))
        ...
        const [res, fields] = await conn.query('INSERT INTO tbl2_guias_traslado_cab(orden_ref,tipo,id_proveedor_CAB,proveedor,servicio,fec_emision,fec_retorno,costo,observaciones,motivo_traslado,responsable,modelo,marca,producto,destino,id_orden_CAB,id_corte_CAB) VALUES(NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),?,NULLIF(?,""))', [cabecera.orden_ref, cabecera.tipo, cabecera.id_proveedor_CAB, cabecera.proveedor, cabecera.servicio, cabecera.fec_emision, cabecera.fec_retorno, cabecera.costo, cabecera.observaciones, cabecera.motivo_traslado, cabecera.responsable, cabecera.modelo, cabecera.marca, cabecera.producto, cabecera.destino, cabecera.id_orden_CAB || 0, cabecera.id_corte_CAB ?? 1])
  ...
  static async UpdateDisponibleFraccionado(backup_articulos,articulos,orden,tipo,acabados,conn){
    ...
    // tipo == 0 ? 'SUMA' : 'RESTA'
    ////////////////////////////////////
    console.log("backup >:",backup_articulos,"articulos >:",articulos,"orden >:",orden,"tipo >:",tipo,"acabados >:",acabados)
    let p1 = '', p2 = '', p3 = ''
    ...
    console.log("Info de tallas para el update:",infotallas)
    
    console.log("acabados condicional: ",!acabados ? "si" : "no")

    console.log("articulos condicional: ", articulos.length > 0 ? "si" : "no")
  ...
  static async UpdateMasterProduccion(backup_articulos,articulos,orden,conn,tipo,acabados = 0){
    ...
    // console.log("La informacion a trabajar es:",backup_articulos,articulos,orden,tipo == 0 ? 'SUMA' : 'RESTA', acabados)

    let result = null
    const [data_orden] = await conn.execute("select *from tbl2_fases_prod_ordenes where idx = ?",[orden])
    console.log("idx fases_prod_ordenes---->",data_orden[0].fraccionado)
  ...
  static async saveInfoPedidosAvios(data) {
    ...
    try {
      ...
      if (data.id) {
        ...
        const [validacion] = await conn.query(`
          SELECT t3.nom,t4.nom as colorsubpro,t2.producto,t2.color,t2.id_subprod_CAB 
      ...
      if (data.id) {
        ...
        for(let fila of articulos){
          ...
          if(fila.id_producto_CAB == ''){
            ...
            // //console.log("Info de la creacion del subproducto:",resultsubprod)
            if(!resultsubprod.ok) {
              //console.log("INfo de bnsuquda:",fila)
              const [busquedasub] = await conn.query("SELECT *FROM tbl2_subproductos WHERE idx_CAB_PROD = ? AND idx_CAB_COLOR = ? AND idx_talla = ?",[fila.idx_producto,fila.idx_color,26])
              fila.id_subprod_CAB = busquedasub[0].idx
            } else {
              fila.id_subprod_CAB = resultsubprod.resultid
            }
            // fila.id_producto_CAB = resultsubprod.resultid
          }
          // try {
          ...
          if (fila.idx && fila.idx !== '') {
            //console.log("Dentro de 1 actualizacion",fila)
            const [results, fields] = await conn.query('UPDATE tbl2_pedidos_insumos_det SET id_pedido_CAB=NULLIF(?, ""),id_producto_CAB=NULLIF(?, ""),id_subprod_CAB=NULLIF(?, ""),producto=NULLIF(?, ""),color=NULLIF(?, ""),rollos=NULLIF(?, ""),cantidad=NULLIF(?, ""),unidad=NULLIF(?, ""),precio=NULLIF(?, ""),anulado=NULLIF(?, ""),modelo=NULLIF(?, ""),corte=NULLIF(?, "") WHERE idx = ?', [parseInt(data.id), fila.idx_producto, fila.id_subprod_CAB ?? null, fila.producto, fila.color.trim(), fila.rollos, fila.cantidad, fila.unidad, fila.precio, fila.anulado, fila.modelo, fila.corte, fila.idx]);

          } else {
            //console.log("Dentro de 2 insertado")
            const [results, fields] = await conn.query('INSERT INTO tbl2_pedidos_insumos_det(id_pedido_CAB,id_subprod_CAB,id_producto_CAB,producto,color,rollos,cantidad,unidad,precio,anulado,modelo,corte) VALUES(NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""))', [parseInt(data.id),fila.id_subprod_CAB ?? null, fila.idx_producto, fila.producto, fila.color.trim(), fila.rollos, fila.cantidad, fila.unidad, fila.precio, fila.anulado, fila.modelo, fila.corte]);
          }
        }

        let [validacion] = await conn.query(`
          ...
          WHERE t1.idx = ?
        `,[parseInt(data.id)])
        // //console.log("La reviio del cruce es:",validacion)
        if(!validacion.length || validacion.map(row=>row.colorsubprod !== row.color).length > 0) throw new Error("Se detectaron inconsistencias durante la generacion de la orden de pedido. Por favor verifique.")
        ...
        for(let fila of articulos){
          ...
          if(cabecera.tipo == 'TELAS'){
            ...
            if(fila.id_producto_CAB == ''){
              //console.log("Dentro de creacion de nuevo subproducto")
              let [newprod] = await conn.query(`SELECT *FROM tbl2_productos WHERE ruc_ = '20522094120' AND idx = ?`,[fila.idx_producto])
              //console.log("La informacion del producto consultado es:",newprod)
              let resultsubprod = await ProductosService.createNewSubProduct({idx_CAB_PROD:fila.idx_producto,codigo:newprod[0].codigo,isbn:newprod[0].isbn,nom:newprod[0].nom,idx_CAB_COLOR:fila.idx_color,idx_talla:26,talla:'S/T',estado:'primera',nro_lote:res.insertId},conn)
              // if(!resultsubprod.ok) throw new Error(resultsubprod.message)
        ...
        `,[res.insertId])
        //console.log("La reviio del cruce es:",validacion)
        if(!validacion.length || validacion.filter(row=>row.colorsubprod !== row.color).length > 0) throw new Error("Se detectaron inconsistencias durante la generacion de la orden de pedido. Por favor verifique.")
  ...
  static async getInfoCuadreTelas(id) {
    let conn
    try {
      conn = await mysql.createConnection(configs[1])
      await conn.connect();
      const [results, fields] = await conn.query(`
        select
          t2.*,
          COALESCE((
            select JSON_ARRAYAGG(JSON_OBJECT('nroguia',ta.nro_guia,'fec_despacho',ta.fec_despacho,'despacho',tb.despacho,'costo',tb.precio)) as info 
            from tbl2_despachos_cab ta
            join tbl2_despachos_det tb on ta.idx = tb.id_despacho_CAB
            where ta.id_pedido_origen = 383 and tb.id_subprod = t2.id_subprod_CAB
          ),JSON_ARRAY()) as ingresos
        from tbl2_pedidos_insumos_cab t1
        join tbl2_pedidos_insumos_det t2 on t1.idx = t2.id_pedido_CAB
        where t1.idx = ?
      `, [id]);
  ...
  static async getListaDespachos(tipo, search) {
    ...
    try {
      ...
      console.log("La consulta extra queda de la siguiente manera:", extra)
  ...
  static async saveInfoDespachosGuia(data) {
    console.log("La informacion recibida para guardar es la siguiente:",data)
    let conn
    const results = { ok: true, message: 'test' }
    const cabecera = JSON.parse(data.info)
    const articulos = JSON.parse(data.detalle)
    ...
    try {
      ...
      if (data.id) {
        ...
        for(let fila of [...articulos]){
          ...
          if (fila.idx && fila.idx !== '') {
            ...
            const [results, fields] = await conn.query('INSERT INTO tbl2_despachos_det(id_despacho_CAB,id_item,despacho,caidos,incompletos) VALUES(NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""))', [parseInt(data.id), fila.id_item, fila.despacho, fila.caidos, fila.incompletos]);
            id_det = results.insertId
          }
      ...
      if(cabecera.tipo == 'SERVICIOS' && parseInt(cabecera.fase)){
        ...
        // console.log("-----TEST Param 01: -----",param1)
        // console.log("-----TEST Param 02: -----",param2)
        // console.log("-----TEST Param orden: -----",info_orden[0].id_orden_CAB)
        // console.log("-----TEST Conexion: -----",conn)
  ...
  static async updateRecepcionAcabados(data) {
    let conn
    const results = { ok: true, message: 'test' }
    const cabecera = JSON.parse(data.info)
    const articulos = JSON.parse(data.detalle)
    //console.log("Dentro de actualizacion de recepcion de acabados")
    // const facturas = JSON.parse(data.facturas)
  ...
  static async getInfoDespachoEmpaquetadoDet__(id, idorden) {
    let conn
    try {
      conn = await mysql.createConnection(configs[1])
      await conn.connect();
      ...
      const [despachos] = await conn.query(`
        select cc.id_combo,sum(cc.despachos) as despacho,sum(cc.caidos) as caidos,sum(cc.incompletos) as incompletos,
        JSON_ARRAYAGG(JSON_OBJECT('talla',cc.talla,'despachos',cc.despachos,'caidos',cc.caidos,'incompletos',cc.incompletos)) as fracciones_despacho from
        (
        select t2.id_combo,t3.talla,sum(t3.despachos) as despachos,sum(t3.caidos) as caidos,sum(t3.incompletos) as incompletos from tbl2_despachos_cab t1
        join tbl2_despachos_det t2 on t1.idx = t2.id_despacho_cab
        join tbl2_despachos_det_fracciones t3 on t2.idx = t3.id_despacho_DET
        where t1.idx = ? -- t1.id_guia_origen = 1360
        group by t2.id_combo,t3.talla
        ) as cc
        group by cc.id_combo
      `,[id]);
      ...
      const consolidado = data_formateado.reduce((c,v)=>{
        if(despachos.map(row=>row.id_combo).includes(v.id_combo)){
          const filtro = despachos.filter(row=>row.id_combo == v.id_combo)[0]
          c.push({...v,despacho:filtro.despacho,caidos:filtro.caidos,incompletos:filtro.incompletos,fracciones_despacho:filtro.fracciones_despacho})
        }
        return c
      },[]) 

      //console.log("El resultado es:",consolidado,consolidado[0].fracciones_despacho)
  ...
  static async getInfoDespachoEmpaquetadoDet(id,idorden) {
    let conn
    let new_articulos = null
    try {
      conn = await mysql.createConnection(configs[1])
      ...
      const [despachos] = await conn.query(`
        ...
        from tbl2_despachos_cab tdc 
        join tbl2_despachos_det tdd on tdc.idx = tdd.id_despacho_CAB 
        where tdc.idx <> ? and tdc.tipo = 'EMPAQUETADO'
      `,[id]);

      const [despachos2] = await conn.query(`
        select cc.idx,cc.id_combo,sum(cc.despachos) as despacho,sum(cc.caidos) as caidos,sum(cc.incompletos) as incompletos,
        JSON_ARRAYAGG(JSON_OBJECT('talla',cc.talla,'cantidad',cc.despachos,'caidos',cc.caidos,'incompletos',cc.incompletos)) as fracciones_despacho from
        (
        select t2.idx,t2.id_combo,t3.talla,sum(t3.despachos) as despachos,sum(t3.caidos) as caidos,sum(t3.incompletos) as incompletos from tbl2_despachos_cab t1
        join tbl2_despachos_det t2 on t1.idx = t2.id_despacho_cab
        join tbl2_despachos_det_fracciones t3 on t2.idx = t3.id_despacho_DET
        where t1.idx = ? -- t1.id_guia_origen = 1360
        group by t2.idx,t2.id_combo,t3.talla
        ) as cc
        group by cc.idx,cc.id_combo
      `,[id]);
      ...
      let consolidado = data_formateado.reduce((carry,value)=>{
        ...
        if(despachos2.map(row=>row.id_combo).includes(value.id_combo)){
          const filtro = despachos2.filter(row=>row.id_combo == value.id_combo)[0]
          carry.push({...value,idx:filtro.idx,despacho:filtro.despacho,caidos:filtro.caidos,incompletos:filtro.incompletos,fracciones_despacho:filtro.fracciones_despacho})
        }
        return carry
      },[])
