  ...
  static async updatePrestamo(data) {
    ...
    try {
      ...
      if (cabecera.idx) {
        ...
        try {
          const [res, fields] = await conn.query('INSERT INTO tbl2_prestamos_cab(ruc_,tipo_tasa_interes,id_proveedor_CAB,proveedor,moneda,tcea,plazo_pago,numero_cuotas,fec_solicutud,fec_ultimo_vencimiento,monto_capital,monto_intereses,monto_prestamo,estado_prestamo) VALUES(NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""),NULLIF(?, ""))', ['20522094120', cabecera.tipo_tasa_interes, cabecera.id_proveedor_CAB, cabecera.proveedor, cabecera.moneda, cabecera.tcea, cabecera.plazo_pago, cabecera.numero_cuotas, cabecera.fec_solicutud, cabecera.fec_ultimo_vencimiento, cabecera.monto_capital, cabecera.monto_intereses, cabecera.monto_prestamo, cabecera.estado_prestamo])
